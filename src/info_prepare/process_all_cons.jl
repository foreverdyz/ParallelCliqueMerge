...
function process_all_cons_new(
    ...
    for i in 1:m
        ...
        if con_ub[i] != Inf
            #convert constraint to PBC (pure binary constraint)
            ...
            if length(bin_con) > 1
                #check whether it is a set pack or conflicting knapsacks
...
function _check_conflict(coef::AbstractVector, b::Real)
    ...
    (is_setpack) && (return 2)
    (sum(partialsort(coef, rev = true, 1:2)) > b) ? (return 1) : (return 0)
end