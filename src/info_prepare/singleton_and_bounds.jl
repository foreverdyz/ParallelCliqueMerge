...
#this function is try to remove singleton and update variables' bounds with simple operations
function singleton_and_bounds(A::AbstractSparseArray, con_ub::AbstractVector, con_lb::AbstractVector, var_ub::AbstractVector, var_lb::AbstractVector, var_type::AbstractVector)
    ...
    for j in 1:m
        ...
        elseif length(findnz(A[j, :])[1]) > 1
            #j is not a singleton constraint; thus push it to the index set I
            push!(I, j)
        end
    end
    #update variables' bounds with simple operations
    for j in I
...
#for one constraint with upper bound, update the variables' bounds
...
#for one constraint with lower bound, update the constraints' bounds
