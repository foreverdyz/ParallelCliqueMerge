...
function clique_extension(set::Vector{Vector{Int64}}, cg::SparseMatrixCSC, numthreads::Int64, max_nonzero::Int64)
    if numthreads > 1
        return _clq_extend_parallel(set, cg, numthreads, max_nonzero)
    else
        return _clq_extend(set, cg, max_nonzero)
    end
end

function _clq_extend_parallel(set::Vector{Vector{Int64}}, cg::SparseMatrixCSC, numthreads::Int64, max_nonzero::Int64)
            ...
            for j in st:en
                ...
                if S[id] > max_nonzero
                  con_set_g[i] = set[i]
                    ...
                    C, pos, s = _extend_unique_clique(set[i], cg, max_nonzero)
                    con_set_g[i] = C[pos]
...
function _clq_extend(set::Vector{Vector{Int64}}, cg::SparseMatrixCSC, max_nonzero::Int64)
    m = length(set)
...
function _extend_unique_clique(c::AbstractVector, cg::SparseMatrixCSC, max_nonzero::Int64)
    #degree claculate
    ...
    (s < max_nonzero) ? (extend_list = [[candidate_list[1]]]) : (return [c], 1, s)
